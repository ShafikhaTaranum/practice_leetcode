...
## Solution Approach
The challenge here is that the array may contain duplicates, which makes binary search more tricky compared to the variant without duplicates. We need to handle cases where nums[mid] == nums[left] == nums[right], which makes it impossible to determine which side is sorted.
